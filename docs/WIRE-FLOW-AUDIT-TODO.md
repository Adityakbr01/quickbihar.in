...
| **Rider** | Web dashboard **and** mobile | web `/delivery/dashboard`, mobile `/rider` + `(tabs)/clothing/rider` | ✅ Full lifecycle wired. ✅ Web active-order panel status strings fixed (H6, `91a1e1b`). ✅ Real proof-of-delivery on web + mobile (M7). |
...
- [x] ✅ **FIXED — M3. Web has no token-refresh flow.** `web/src/lib/axios.ts` just rejected on 401 — no `/auth/refresh-token` call, no queue (mobile did this correctly), so web users were silently logged out on access-token expiry. **Fix applied:** ported the mobile silent-refresh interceptor to web — a 401 (other than from the refresh endpoint itself) triggers exactly one `POST /auth/refresh-token` (refresh token rides as the httpOnly cookie, `withCredentials`), concurrent 401s queue in `failedQueue` and replay with the new token, and the persisted zustand access token (`admin-auth-storage`) is patched in place. Terminal failures clear auth and bounce to the portal-appropriate login (`/admin`, `/seller`, `/delivery`). Typecheck clean.
- [x] ✅ **FIXED — M4. No server-side route protection on web.** Guards were client-only; no server middleware, so the dashboard shell/JS shipped to anonymous visitors. **Fix applied:** added `web/src/proxy.ts` (Next.js 16 renamed the `middleware` convention to `proxy` — verified against the installed `next@16.2.3` docs: single `export function proxy(request: NextRequest)` + `export const config.matcher`, in `src/`). Uses the httpOnly `accessToken` cookie as a **presence** signal only — unauthenticated hits to `/{admin,seller,delivery}/dashboard` bounce to that portal's login; authenticated hits to a login page bounce to the dashboard. Deliberately **not** the security boundary (the JWT carries no role claim; per-role authz stays in backend RBAC on every `/api` call) — this is defense-in-depth + UX so the shell isn't served to anonymous visitors. Matcher scoped to the three portals; landing/_next/api/static skipped.
- [x] ✅ **FIXED — M5. `RIDER_OFFER_CLOSED` never emitted/consumed.** When one rider accepted, competing riders kept a stale, un-acceptable offer modal (server had the constant but never emitted; web/mobile never listened). **Fix applied:** server now emits `RIDER_OFFER_CLOSED` in two places — (a) `riderAcceptOrder` (`subOrder.service.ts`) captures every competing OPEN offer before flipping it to EXPIRED and pushes `{ offerId, subOrderId, reason: "TAKEN", message }` to each losing rider; (b) `processMatchingPool` (`matching.service.ts`) captures timing-out offers before the bulk EXPIRED write and emits `reason: "EXPIRED"` so on-the-clock expiries also clear. Consumers: web `delivery/dashboard/page.tsx` clears `activeJobOffer` (functional update, matched by offerId/subOrderId) + toasts; mobile `RiderWorkspaceScreen.tsx` removes the offer from state, clears its notified-id, and dismisses the open "New Delivery Offer" dialog. Constant already shared across all three `socketEvents.ts`. Server + web typecheck clean; mobile clean except pre-existing `@hugeicons` typing noise.
- [x] ✅ **FIXED — M6. Web missing `NEW_NOTIFICATION` listener.** Server emitted it and mobile listened, but web omitted the constant entirely → web admin got no realtime new-notification push. **Fix applied:** added `NEW_NOTIFICATION` (+ the sibling `NOTIFICATION_UPDATED` / `NOTIFICATION_STATUS_UPDATE`) to web `constants/socketEvents.ts` and registered a `handleNewNotification` listener in `SocketListenerProvider.tsx` (on/off symmetric). Typecheck clean.
- [x] ✅ **FIXED — M7. Rider proof-of-pickup/delivery is a stub.** Web used free-text URL inputs (hand-typed/hardcoded ImageKit URL, "Mock photo auto-injected") and a plain-text signature field; mobile captured a real camera photo but submitted the device-local `file://` URI, which is unreachable to the rest of the platform (`ActiveOrdersPanel.tsx:318,328,400-417`, `riderMedia.ts:29`). **Fix applied:** new authenticated endpoint `POST /delivery/proof-upload` (`upload.single("file")` → `uploadToImageKit(..., "delivery-proofs")` → `{ url, fileId }`) is the single upload path for both clients. Web `ActiveOrdersPanel` replaces the URL fields with a `ProofPhotoInput` widget (hidden `capture="environment"` file input, uploads on select, loader + thumbnail preview) for pickup & delivery, plus a native `<canvas>` `SignaturePad` (pointer/touch draw, clear, `toBlob` → PNG upload); pickup/deliver now hard-require a real hosted photo URL (guarded before mutate), signature stays optional. Mobile `pickProofPhoto` now uploads the captured image (RN `FormData` `{ uri, name, type }`) via the new `deliveryApi.uploadProof(uri, kind)` and returns the **hosted** URL, surfacing upload failures through the rider dialog; both JobsPanel call sites tag `kind`. Client upload timeout bumped to 30s for large images. Server + web typecheck clean; mobile clean except pre-existing `@hugeicons`/`LottieView` typing noise.
- [x] ✅ **FIXED — M8. Prefilled real demo credentials in web login forms.** `SellerLoginForm.tsx` / `DeliveryLoginForm.tsx` shipped real working credentials as `defaultValues`. **Fix applied:** both forms now default to empty `email`/`password`; the real address is gone, leaving only a non-functional `*@example.com` placeholder.
- [x] ✅ **FIXED — M9. Hardcoded plain-HTTP API origin (mobile).** `axiosInstance.ts` fell back to `http://80.225.194.37` (no TLS, raw IP) when the env var was unset. **Fix applied:** the hardcoded fallback is removed — `API_ORIGIN` now derives solely from `process.env.EXPO_PUBLIC_API_ORIGIN` (empty string if unset, surfacing a loud "Network Error: could not reach server" rather than silently shipping cleartext to a raw IP); production supplies an HTTPS domain via env.
- [x] ✅ **FIXED — M10. Matching score is effectively distance-only.** Rating/acceptance/completion/onlineHours were hardcoded constants (`4.5 / 0.9 / 0.95 / 4.0`) summing to a fixed `27.9` added to every rider, so only `distanceScore * 0.4` ever changed the ranking; and `logNoQualifiedRiders` ran 4 live aggregations then discarded everything (its entire body was a commented-out `console.warn`). **Fix applied:** the rider model tracks none of those reputation signals, so the formula was collapsed to `score = max(0, 10 - riderDistanceToStore)` (proximity only) with a comment explaining what was removed and why, to be re-weighted if/when the signals actually exist. Deleted the dead `logNoQualifiedRiders` function, its two call sites (each already preceded by a real `console.error`/`console.log` + `return`), and the now-orphaned `validCoordsFromGeoJson` helper and `lastDiagnosticLogAt` map. The shared `busyRiderUserIds` / `riderCapacityCountsByRider` / `rejectionCountsForSubOrder` helpers stay (still used by the live `diagnostics()` endpoint). Server typecheck clean.
- [x] ✅ **FIXED — M11. Inconsistent role parsing (mobile).** `admin.tsx` used only `user?.role?.name` while `_layout.tsx`/`rider.tsx` handled both the string and object forms → the admin tab could show but the screen redirect away. **Fix applied:** centralized a `getRoleName(role)` helper in `authStore.ts` that normalizes both shapes, and routed the call sites (incl. the persisted `userRole`) through it.
