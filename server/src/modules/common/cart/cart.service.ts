import * as cartDAO from "./cart.dao";
import * as ProductDAO from "@/modules/clothing/products/product.dao";
import * as appConfigService from "@/modules/common/appConfig/appConfig.service";
import { ApiError } from "@/utils/ApiError";

/**
 * Retrieves a user's cart with fully computed, display-ready pricing.
 * Populates product data, calculates GST-inclusive prices, per-item stock status,
 * subtotal, total tax, and applies dynamic shipping rules from app configuration.
 * Invalid items (whose product no longer exists) are filtered out of the response.
 *
 * @param userId - Owning user's MongoDB ObjectId (as string).
 * @returns Cart summary containing items, subtotal, totalTax, shippingFee,
 *          freeShippingThreshold, and itemCount. Returns an empty-cart shape
 *          (subtotal 0, no items) when the user has no cart.
 */
export async function getCart(userId: string) {
    const cart = await cartDAO.findByUserId(userId);

    // Fetch Shipping Configuration
    const config = await appConfigService.getConfig();
    const shippingRules = config?.shipping || { freeShippingThreshold: 2000, shippingFee: 99 };

    if (!cart) return {
        userId,
        items: [],
        subtotal: 0,
        totalTax: 0,
        shippingFee: 0,
        freeShippingThreshold: shippingRules.freeShippingThreshold
    };

    // Calculate dynamic subtotal and filter out invalid items if necessary
    let subtotal = 0;
    let totalTax = 0;

    const validatedItems = cart.items.map((item: any) => {
        const product = item.productId;
        if (!product) return null;

        const variant = product.variants.find((v: any) => v.sku === item.sku);

        // Calculate GST-inclusive price
        const basePrice = product.price;
        const itemPrice = Math.round(product.isGstApplicable
            ? basePrice * (1 + (product.gstPercentage || 0) / 100)
            : basePrice);

        const itemTax = Math.round(itemPrice - basePrice);

        subtotal += itemPrice * item.quantity;
        totalTax += itemTax * item.quantity;

        return {
            ...item.toObject(),
            productTitle: product.title,
            price: itemPrice,
            image: product.images[0]?.url,
            selectedSize: variant?.size,
            selectedColor: variant?.color,
            stockStatus: variant ? (variant.stock >= item.quantity ? "IN_STOCK" : "LOW_STOCK") : "OUT_OF_STOCK",
            availableStock: variant?.stock || 0,
            sellerId: product.sellerId,
            storeId: product.storeId,
        };
    }).filter(Boolean);

    const shippingFee = subtotal >= shippingRules.freeShippingThreshold ? 0 : shippingRules.shippingFee;

    return {
        _id: cart._id,
        userId: cart.userId,
        items: validatedItems,
        subtotal,
        totalTax: Math.round(totalTax),
        shippingFee,
        freeShippingThreshold: shippingRules.freeShippingThreshold,
        itemCount: validatedItems.length
    };
}

/**
 * Adds a product variant to the user's cart, or increments the quantity if the
 * variant already exists. Validates product existence, SKU validity, and stock
 * availability (including the combined quantity for pre-existing items).
 * Lazily creates a cart for the user if none exists.
 *
 * @param userId - Owning user's MongoDB ObjectId (as string).
 * @param productId - MongoDB ObjectId of the product being added.
 * @param sku - Variant SKU identifying the specific size/color combination.
 * @param quantity - Units to add. Defaults to 1.
 * @returns The updated cart document.
 * @throws {ApiError} 404 if the product or variant (SKU) is not found.
 * @throws {ApiError} 400 if requested quantity exceeds available stock.
 * @throws {ApiError} 500 if the cart cannot be created or fetched.
 */
export async function addToCart(userId: string, productId: string, sku: string, quantity: number = 1) {
    // 1. Verify Product and Stock
    const product = await ProductDAO.findById(productId);
    if (!product) throw new ApiError(404, "Product not found");

    const variant = product.variants.find(v => v.sku === sku);
    if (!variant) throw new ApiError(404, "Specific variant (SKU) not found");

    if (variant.stock < quantity) {
        throw new ApiError(400, `Only ${variant.stock} items available in stock`);
    }

    // 2. Fetch/Create Cart
    let cart = await cartDAO.findRawByUserId(userId);
    if (!cart) {
        cart = await cartDAO.create(userId);
    }

    if (!cart) throw new ApiError(500, "Failed to create/fetch cart");

    // 3. Update Items Logic
    const existingItem = cart.items.find(item => item.sku === sku);
    if (existingItem) {
        const newQuantity = existingItem.quantity + quantity;
        if (variant.stock < newQuantity) {
            throw new ApiError(400, `Total quantity (${newQuantity}) exceeds available stock (${variant.stock})`);
        }
        existingItem.quantity = newQuantity;
    } else {
        cart.items.push({ productId: productId as any, sku, quantity });
    }

    return await cartDAO.updateItems(userId, cart.items);
}

/**
 * Sets an absolute quantity for an existing cart item, re-validating stock.
 *
 * @param userId - Owning user's MongoDB ObjectId (as string).
 * @param sku - Variant SKU of the item to update.
 * @param quantity - The new absolute quantity for the item.
 * @returns The updated cart document.
 * @throws {ApiError} 404 if the cart or the target item is not found.
 * @throws {ApiError} 400 if the new quantity exceeds available stock.
 */
export async function updateQuantity(userId: string, sku: string, quantity: number) {
    const cart = await cartDAO.findRawByUserId(userId);
    if (!cart) throw new ApiError(404, "Cart not found");

    const item = cart.items.find(i => i.sku === sku);
    if (!item) throw new ApiError(404, "Item not found in cart");

    // Verify stock for new quantity
    const product = await ProductDAO.findById(item.productId.toString());
    const variant = product?.variants.find(v => v.sku === sku);
    if (variant && variant.stock < quantity) {
        throw new ApiError(400, `Only ${variant.stock} items available`);
    }

    item.quantity = quantity;
    return await cartDAO.updateItems(userId, cart.items);
}

/**
 * Removes a single variant from the user's cart by SKU.
 *
 * @param userId - Owning user's MongoDB ObjectId (as string).
 * @param sku - Variant SKU of the item to remove.
 * @returns The updated cart document.
 * @throws {ApiError} 404 if the cart is not found.
 */
export async function removeItem(userId: string, sku: string) {
    const cart = await cartDAO.findRawByUserId(userId);
    if (!cart) throw new ApiError(404, "Cart not found");

    cart.items = cart.items.filter(i => i.sku !== sku);
    return await cartDAO.updateItems(userId, cart.items);
}

/**
 * Empties the user's cart entirely.
 *
 * @param userId - Owning user's MongoDB ObjectId (as string).
 * @returns The cleared cart document, or `null` if no cart exists.
 */
export async function clearCart(userId: string) {
    return await cartDAO.clearCart(userId);
}

/**
 * Merges a client-side (guest) cart into the user's server cart.
 * For each incoming item, increments the quantity if the SKU already exists,
 * otherwise appends it. Lazily creates a cart if none exists.
 *
 * @param userId - Owning user's MongoDB ObjectId (as string).
 * @param items - Items to merge, each with productId, sku, and quantity.
 * @returns The updated cart document.
 * @throws {ApiError} 500 if the cart cannot be created or fetched.
 */
export async function syncCart(userId: string, items: { productId: string; sku: string; quantity: number }[]) {
    let cart = await cartDAO.findRawByUserId(userId);
    if (!cart) {
        cart = await cartDAO.create(userId);
    }

    if (!cart) throw new ApiError(500, "Failed to create/fetch cart");

    for (const item of items) {
        const existingItem = cart.items.find(i => i.sku === item.sku);
        if (existingItem) {
            existingItem.quantity += item.quantity;
        } else {
            cart.items.push({ productId: item.productId as any, sku: item.sku, quantity: item.quantity });
        }
    }

    return await cartDAO.updateItems(userId, cart.items);
}
