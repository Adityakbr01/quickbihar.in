/**
 * App-config data-access layer.
 *
 * Wraps the singleton `AppConfig` document. The application only ever keeps one config row,
 * so reads target the first document and writes upsert against it — that invariant lives here.
 */
import { AppConfig } from "./appConfig.model";
import type { UpdateAppConfigBody } from "./appConfig.validation";

/** Fetch the single config document, or null if it has not been created yet. */
export async function getConfig() {
    // We always fetch the first document.
    // If it doesn't exist, we return null (service will handle initial creation).
    return await AppConfig.findOne();
}

/** Upsert the singleton config: update the existing row if present, otherwise create it. */
export async function updateConfig(data: UpdateAppConfigBody & { updatedBy?: string }) {
    // Find existing or create new if not exists (upsert)
    // Since we don't have a unique key other than _id,
    // we'll findOne and if exists update it, else create it.
    const config = await AppConfig.findOne();
    if (config) {
        return await AppConfig.findByIdAndUpdate(config._id, data, { returnDocument: 'after' });
    } else {
        return await AppConfig.create(data);
    }
}
