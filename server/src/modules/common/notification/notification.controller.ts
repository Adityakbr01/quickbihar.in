 ...
 * @route POST /api/v1/notifications/batch-delete
 * @access Protected (admin)
 */
export const batchDeleteNotifications = asyncHandler(async (req: Request, res: Response) => {
  const { ids } = req.body;
 ...
 * @access Protected
 */
export const reportDelivery = asyncHandler(async (req: Request, res: Response) => {
 ...
 * @route PATCH /api/v1/notifications/:id/opened
 * @access Protected
 */
export const reportOpen = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  const user = (req as any).user;
  if (!user) {
    throw new ApiError(401, "Authentication required");
 ...
 * @route POST /api/v1/notifications/test-direct-push
 * @access Protected (admin)
 */
export const testDirectPush = asyncHandler(async (req: Request, res: Response) => {
  const {
    targetUser,
    targetToken,
    title,
