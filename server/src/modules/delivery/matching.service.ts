...
import { distanceKmBetween, calculateRiderPayout } from "../order/subOrder.service";
...
export class MatchingService {
    ...
    static async initiateMatching(subOrderId: string) {
        const subOrder = await SubOrder.findById(subOrderId).populate("storeId parentOrderId");
        if (subOrder && subOrder.status === SubOrderStatus.READY_FOR_PICKUP && !subOrder.delivery.riderId) {
            await this.matchSubOrder(subOrder);
        }
    }

    static matchingStage(subOrder: any) {
        const pickupTime = subOrder.packageDetails?.pickupTiming || subOrder.updatedAt || subOrder.createdAt;
        const elapsedMs = Date.now() - new Date(pickupTime).getTime();
        const elapsedSeconds = Math.max(0, elapsedMs / 1000);

        if (elapsedSeconds > 120) {
            return { elapsedSeconds, stage: 4, radiusMeters: ENV.MATCHING_STAGE4_RADIUS_KM * 1000, expiresInSeconds: 60 };
        }
        if (elapsedSeconds > 60) {
            return { elapsedSeconds, stage: 3, radiusMeters: 8000, expiresInSeconds: 60 };
        }
        if (elapsedSeconds > 30) {
            return { elapsedSeconds, stage: 2, radiusMeters: 5000, expiresInSeconds: 30 };
        }
        return { elapsedSeconds, stage: 1, radiusMeters: 3000, expiresInSeconds: 30 };
    }

    static async diagnostics(subOrderId: string) {
        const subOrder = await SubOrder.findOne({
            $or: [
                ...(Types.ObjectId.isValid(subOrderId) ? [{ _id: new Types.ObjectId(subOrderId) }] : []),
                { subOrderId },
            ],
        ...
        const storeCoords = Number.isFinite(Number(storeLat)) && Number.isFinite(Number(storeLng))
        ...
        const capacityBlockedSet = new Set(capacitySummary.atCapacityIds.map((id) => this.idString(id)));
        const rejectionSummary = await this.rejectionCountsForSubOrder(subOrder._id);
        const rejectionBlockedSet = new Set(rejectionSummary.blockedIds.map((id) => this.idString(id)));
        ...
        const riders = onlineRiders.map((rider: any) => {
            const riderUserId = this.idString(rider.userId?._id || rider.userId);
            const rejectionCount = rejectionSummary.countByRiderId.get(riderUserId) || 0;
            const acceptedCountInWindow = capacitySummary.countByRiderId.get(riderUserId) || 0;
            const riderCoords = {
                latitude: Number(rider.currentLocation?.coordinates?.[1]),
                longitude: Number(rider.currentLocation?.coordinates?.[0]),
            };
            const distanceKm = storeCoords ? distanceKmBetween(riderCoords, storeCoords) : null;
            return {
                ...
                withinCurrentRadius: distanceKm === null ? false : distanceKm * 1000 <= stage.radiusMeters,
            };
        }).sort((a, b) => (a.distanceKm ?? Number.POSITIVE_INFINITY) - (b.distanceKm ?? Number.POSITIVE_INFINITY));

        const openOffers = await RiderOffer.find({ subOrderObjectId: subOrder._id, status: "OPEN" })
            .select("offerId riderId stage radiusKm payoutAmount expiresAt createdAt")
            .lean();

        const blockers: string[] = [];
        if (subOrder.status !== SubOrderStatus.READY_FOR_PICKUP) blockers.push(`Sub-order status is ${subOrder.status}`);
        if (subOrder.delivery?.riderId) blockers.push("Sub-order already has an assigned rider");
        if (!storeCoords) blockers.push("Store pickup GPS is missing or invalid");
        if (!onlineRiders.length) blockers.push("No approved online riders with GPS are available");
        if (storeCoords && onlineRiders.length && !riders.some((rider) => rider.withinCurrentRadius)) {
            blockers.push(`No online riders within ${stage.radiusMeters / 1000} km of store pickup GPS`);
        }
        if (
            storeCoords
            && riders.some((rider) => rider.withinCurrentRadius)
            && !riders.some((rider) => rider.withinCurrentRadius && !rider.atAcceptanceCapacity)
        ) {
            blockers.push(`Nearby riders are within radius, but all are at the ${capacitySummary.maxAcceptedOrders} accepted order limit for ${capacitySummary.windowHours} hours`);
        }
        if (
            storeCoords
            && riders.some((rider) => rider.withinCurrentRadius && !rider.atAcceptanceCapacity)
            && !riders.some((rider) => rider.withinCurrentRadius && !rider.atAcceptanceCapacity && !rider.offerSuppressedForSubOrder)
        ) {
            blockers.push(`Nearby available riders have already rejected this sub-order ${MAX_RIDER_REJECTIONS_PER_SUB_ORDER} times`);
        }
        if (!openOffers.length && !blockers.length) {
            blockers.push("Matching is eligible, but no open offers exist yet. Retry matching or wait for the polling loop.");
        }
        ...
        return {
            ...
            riders: {
                ...
                atAcceptanceCapacity: riders.filter((rider) => rider.atAcceptanceCapacity).length,
                suppressedByRejections: riders.filter((rider) => rider.offerSuppressedForSubOrder).length,
                withinCurrentRadius: riders.filter((rider) => rider.withinCurrentRadius).length,
                eligibleWithinCurrentRadius: riders.filter((rider) => rider.withinCurrentRadius && !rider.atAcceptanceCapacity && !rider.offerSuppressedForSubOrder).length,
    ...
    private static async logNoQualifiedRiders(input: {
        ...
        const key = input.subOrder?._id?.toString?.() || input.subOrder?.subOrderId || "unknown";
        const now = Date.now();
        const last = this.lastDiagnosticLogAt.get(key) || 0;
        if (now - last < 30000) return;
        this.lastDiagnosticLogAt.set(key, now);

        const storeCoords = this.validCoordsFromGeoJson(input.store?.currentLocation);
        ...
        const capacitySummary = await riderCapacityCountsByRider();
        const capacityBlockedSet = new Set(capacitySummary.atCapacityIds.map((id) => this.idString(id)));
        const rejectionSummary = await this.rejectionCountsForSubOrder(input.subOrder._id);
        const rejectionBlockedSet = new Set(rejectionSummary.blockedIds.map((id) => this.idString(id)));
        ...
        const riderDiagnostics = approvedOnlineRiders
            .map((rider: any) => {
                ...
                const distanceKm = riderCoords && storeCoords ? distanceKmBetween(riderCoords, storeCoords) : null;
                return {
                    profileId: rider._id?.toString(),
                    userId: riderUserId,
                    email: rider.userId?.email,
                    ...
                    withinRadius: distanceKm === null ? false : distanceKm * 1000 <= input.radiusMeters,
                };
            })
            .sort((a, b) => (a.distanceKm ?? Number.POSITIVE_INFINITY) - (b.distanceKm ?? Number.POSITIVE_INFINITY));
        ...
        if (!approvedOnlineRiders.length) blockers.push("No approved online rider profiles found.");
        if (approvedOnlineRiders.length && !riderDiagnostics.some((rider) => rider.hasGps)) {
            blockers.push("Approved online riders exist, but none have GPS coordinates saved.");
        }
        if (storeCoords && riderDiagnostics.some((rider) => rider.hasGps) && !riderDiagnostics.some((rider) => rider.withinRadius)) {
            blockers.push(`Approved online riders with GPS exist, but none are within ${input.radiusMeters / 1000} km of the store.`);
        }
        if (
            storeCoords
            && riderDiagnostics.some((rider) => rider.withinRadius)
            && !riderDiagnostics.some((rider) => rider.withinRadius && !rider.atAcceptanceCapacity)
        ) {
            blockers.push(`Approved online riders are within radius, but all are at the ${capacitySummary.maxAcceptedOrders} accepted order limit for ${capacitySummary.windowHours} hours.`);
        }
        if (
            storeCoords
            && riderDiagnostics.some((rider) => rider.withinRadius && !rider.atAcceptanceCapacity)
            && !riderDiagnostics.some((rider) => rider.withinRadius && !rider.atAcceptanceCapacity && !rider.offerSuppressedForSubOrder)
        ) {
            blockers.push(`Approved online riders are within radius, but all nearby available riders have already rejected this sub-order ${MAX_RIDER_REJECTIONS_PER_SUB_ORDER} times.`);
        }

        console.warn("[MatchingService] Matching diagnostics", JSON.stringify({
            reason: input.reason,
            subOrder: {
            ...
            customer: {
                ...
                location: customerCoords && Number.isFinite(customerCoords.latitude) && Number.isFinite(customerCoords.longitude)
                    ? customerCoords
                    : null,
            },
            riderSummary: {
                approvedOnline: approvedOnlineRiders.length,
                approvedOnlineVerified: approvedOnlineRiders.filter((rider: any) => rider.isVerified).length,
                approvedOnlineWithGps: riderDiagnostics.filter((rider) => rider.hasGps).length,
                busyWithActiveJob: riderDiagnostics.filter((rider) => rider.hasActiveJob).length,
                atAcceptanceCapacity: riderDiagnostics.filter((rider) => rider.atAcceptanceCapacity).length,
                suppressedByRejections: riderDiagnostics.filter((rider) => rider.offerSuppressedForSubOrder).length,
                withinRadius: riderDiagnostics.filter((rider) => rider.withinRadius).length,
                eligibleWithinRadius: riderDiagnostics.filter((rider) => rider.withinRadius && !rider.atAcceptanceCapacity && !rider.offerSuppressedForSubOrder).length,
            },
    ...
    private static async matchSubOrder(subOrder: any) {
        ...
        if (!store || !store.currentLocation?.coordinates) {
            console.error(`[MatchingService] Store location coordinates missing for sub-order ${subOrder.subOrderId}`);
        ...
        const capacitySummary = await riderCapacityCountsByRider();
        const rejectionSummary = await this.rejectionCountsForSubOrder(subOrder._id);
        const excludedRiderIds = this.objectIdsFrom([...capacitySummary.atCapacityIds, ...rejectionSummary.blockedIds]);
        ...
        const qualifiedRiders = await DeliveryBoy.find({
            status: "APPROVED",
            isVerified: true,
            isOnline: true,
        ...
        if (qualifiedRiders.length === 0) {
            console.log(`[MatchingService] No riders found within ${radiusMeters / 1000} KM radius for sub-order ${subOrder.subOrderId}`);
            await this.logNoQualifiedRiders({
                subOrder,
                store,
                parentOrder,
        ...
        const ridersWithScores = qualifiedRiders.map(rider => {
            ...
            if (riderLng && riderLat) {
                riderDistanceToStore = distanceKmBetween(
            ...
            const acceptanceRate = 0.90; // default fallback
            ...
            const score = (distanceScore * 0.4) + (rating * 2.0) + (acceptanceRate * 10) + (completionRate * 10) + (onlineHours * 0.1);
        ...
        for (const item of ridersWithScores) {
            ...
            const offer = await RiderOffer.findOneAndUpdate(
                ...
                },
                {
                    ...
                    $set: {
                        ...
                        riderDistanceToStoreKm: parseFloat(item.riderDistanceToStore.toFixed(2)),
            ...
            socketService.emitToUser(riderUserId, SocketEvents.RIDER_JOB_OFFER, {
                ...
                riderCapacity: {
                    ...
                    maxAcceptedOrders: capacitySummary.maxAcceptedOrders,
                    acceptanceWindowHours: capacitySummary.windowHours,
                    remainingAfterAccept: Math.max(0, capacitySummary.maxAcceptedOrders - acceptedCountInWindow - 1),
                },
            });
        }

        if (emittedOffers > 0 || existingOpenOffers > 0 || capacitySuppressedOffers > 0 || rejectionSuppressedOffers > 0) {
            console.log(
