...
export function SellerMallPanel({ setup }: { setup?: any }) {
  ...
  if (mallDataLoading) {
    return <div className="py-10 text-center text-sm text-muted-foreground">Loading Mall Panel...</div>;
  }
        ...
        {mallsList.length === 0 ? (
          <Card className="border-border bg-card p-8 text-center">
            <CardContent className="flex flex-col items-center justify-center space-y-4 pt-6">
              ...
              <div>
                <h3 className="text-base font-semibold text-foreground">No malls linked or requested yet</h3>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {mallsList.map((mall: any) => (
              <Card key={mall._id} className="border-border bg-card hover:border-emerald-500/30 transition duration-200">
                <CardHeader className="pb-3 flex flex-row items-start justify-between space-y-0">
                  <div className="flex items-center gap-3">
                    <img 
                      ...
                      className="h-10 w-10 object-cover rounded border border-border bg-muted" 
                      alt={mall.name} 
                    />
                    <div>
                      <h3 className="font-semibold text-foreground text-sm line-clamp-1">{mall.name}</h3>
                      <p className="text-xs text-muted-foreground mt-0.5">{mall.address?.city || "N/A"}, {mall.address?.state || ""}</p>
                    </div>
                  </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-border">
                    ...
                    <Button 
                      ...
                      className="border-border hover:bg-muted text-xs text-muted-foreground hover:text-foreground"
                    >
                      View Details
                    </Button>
                  </div>
  ...
  if (activeMall && !isEditing) {
    return (
      <div className="grid gap-6">
        <div className="flex items-center justify-between">
          <Button variant="outline" className="border-border text-muted-foreground" onClick={() => setSelectedMallId(null)}>
            ← Back to Malls
          </Button>
        </div>
        <Card className="border-border bg-card">
          <CardHeader className="flex flex-row items-center justify-between border-b border-border pb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" variant="outline" className="border-border text-muted-foreground hover:text-foreground" onClick={startEdit}>
            ...
            <div className="grid gap-6 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1 text-sm text-foreground">
                    ...
                    {activeMall.address?.latitude && activeMall.address?.longitude && (
                      <div className="flex items-center gap-2 text-muted-foreground pl-6 pt-1">
              ...
              <div className="space-y-4">
                <div className="flex gap-4">
                  {activeMall.logoUrl && (
                    <div>
                      <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Logo</h4>
                      <img src={activeMall.logoUrl} className="h-20 w-20 object-cover rounded-lg border border-border" alt="Logo" />
                    </div>
                  )}
                  {activeMall.coverImageUrl && (
                    <div className="flex-1">
                      <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Cover</h4>
                      <img src={activeMall.coverImageUrl} className="h-20 w-full object-cover rounded-lg border border-border" alt="Cover" />
                    </div>
                  )}
                </div>

                <div>
                  <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Mall Images ({activeMall.images?.length || 0})</h4>
                  <div className="flex flex-wrap gap-2">
                    {activeMall.images && activeMall.images.length > 0 ? (
                      activeMall.images.map((img: any, idx: number) => (
                        <img key={idx} src={img.url} className="h-16 w-16 object-cover rounded-lg border border-border" alt="Mall" />
                      ))
                    ) : (
                      <p className="text-xs text-muted-foreground italic">No gallery images uploaded yet.</p>
                    )}
                  </div>
                </div>
              </div>
  ...
  if (activeMall && isEditing) {
    return (
      <div className="grid gap-6">
        <div className="flex items-center justify-between">
          <Button variant="outline" className="border-border text-muted-foreground" onClick={() => setIsEditing(false)}>
            ← Cancel Edit
          </Button>
        </div>
        <Card className="border-border bg-card">
            ...
            <form onSubmit={submitEdit} className="grid gap-4">
              ...
              <label className={labelClass}>
                ...
                <textarea
                  ...
                  className={inputClass + " h-20 py-2 px-3 rounded-lg border border-border text-sm"}
                />
              </label>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <label className={labelClass}>
                    ...
                    <input
                      ...
                      className={inputClass + " h-9 px-2 rounded border border-border text-xs"}
                    />
                  </label>
                  <label className={labelClass}>
                    <span>Longitude</span>
              ...
              <div className="border-t border-border pt-3">
                <h3 className="text-sm font-semibold text-muted-foreground mb-2">Contact Info</h3>
                <div className="grid grid-cols-2 gap-2">
                ...
                <div className="grid grid-cols-2 gap-2 mt-3 items-center">
                  ...
                  <label className="flex items-center gap-2 text-xs text-muted-foreground select-none cursor-pointer mt-6">
                    <input
                      type="checkbox"
              ...
              <div className="border-t border-border pt-3">
                <h3 className="text-sm font-semibold text-muted-foreground mb-2">Media & Images</h3>
                <div className="grid grid-cols-2 gap-2 mb-3">
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <label className={labelClass}>
                    ...
                    <input type="file" name="logo" accept="image/*" className={inputClass + " py-1 px-2 rounded border border-border text-xs"} />
                  </label>
                  <label className={labelClass}>
                    <span>Upload Cover Image File</span>
                    <input type="file" name="coverImage" accept="image/*" className={inputClass + " py-1 px-2 rounded border border-border text-xs"} />
                  </label>
                </div>
                ...
                <div className="space-y-3 pt-2">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {existingImages.map((img, idx) => (
                          <div key={idx} className="relative group border border-border rounded overflow-hidden">
                  ...
                  <label className={labelClass}>
                    ...
                    <input
                      ...
                      multiple
                      accept="image/*"
                      className={inputClass + " py-1 px-2 rounded border border-border text-xs"}
                    />
                    <span className="text-[10px] text-muted-foreground normal-case">Note: Select multiple files at once. Total images (existing + new) must be between 1 and 5.</span>
                  </label>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 border-t border-border pt-3">
                ...
                <Button type="button" variant="outline" className="border-border text-foreground" onClick={() => setIsEditing(false)}>
                  Cancel
                </Button>
              </div>
            </form>
    ...
    <div className="grid gap-6">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card className="border-border bg-card xl:col-span-2">
          <CardHeader className="border-b border-border">
            <CardTitle className="text-base text-foreground">Mall Status</CardTitle>
          </CardHeader>
          <CardContent className="pt-4 space-y-4">
            ...
            {currentSetup?.seller.mallRequest && (
              <div className="rounded-lg border border-border bg-muted p-3">
                <div className="text-xs font-medium uppercase text-muted-foreground">Mall Request Status</div>
        ...
        <Card className="border-border bg-card">
          <CardHeader className="border-b border-border">
            <CardTitle className="text-base text-foreground">Connect Existing Mall</CardTitle>
          </CardHeader>
          <CardContent className="pt-4">
        ...
        <Card className="border-border bg-card">
          <CardHeader className="border-b border-border">
            <CardTitle className="text-base text-foreground">Request New Mall</CardTitle>
          </CardHeader>
          <CardContent className="pt-4">
