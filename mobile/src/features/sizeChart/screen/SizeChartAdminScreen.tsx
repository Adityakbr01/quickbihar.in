import SizeChartForm from "@/src/features/sizeChart/components/SizeChartForm";
import SizeChartList from "@/src/features/sizeChart/components/SizeChartList";
import {
  useCreateSizeChart,
  useDeleteSizeChart,
  useSizeCharts,
  useUpdateSizeChart,
} from "@/src/features/sizeChart/hooks/useSizeCharts";
import { ISizeChart } from "@/src/features/sizeChart/types/sizeChart.types";
...
const SizeChartAdminScreen = () => {
  ...
  const [selectedChart, setSelectedChart] = useState<ISizeChart | undefined>(undefined);
  ...
  return (
    <SafeViewWrapper >
      <View style={[SizeChartAdminScreenStyle.header, { borderBottomColor: theme.border }]}>
        <TouchableOpacity onPress={() => router.back()} style={SizeChartAdminScreenStyle.backButton}>
          <HugeiconsIcon icon={ArrowLeft01Icon} size={24} color={theme.text} />
        </TouchableOpacity>
        <Text style={[SizeChartAdminScreenStyle.headerTitle, { color: theme.text }]}>Size Charts</Text>
        <TouchableOpacity
          style={[SizeChartAdminScreenStyle.addButton, { backgroundColor: theme.primary }]}
