 ...
 *
 * Arrives via the deep link in the reset email:
 *   QuickBihar://auth/reset-password?token=...
 * Validates the new password, POSTs /auth/reset-password with the
 * token, then routes the user to Sign In.
 */
export default function ResetPasswordConfirmScreen() {
  ...
  const handleSubmit = () => {
    ...
    if (!params.token) {
      setError(
        "Missing or invalid reset token. Please request a new link from the forgot-password screen."
      );
      return;
    }
    ...
    resetPassword(
      { token: params.token, password },
      {
        onSuccess: () => {
          ...
          setSuccess(true);
          // 2s then bounce to login — the user can use the new password.
          setTimeout(() => {
      ...
      >
        ...
        <View style={styles.successCard}>
          ...
          <Text
            style={[
              styles.subtitle,
              { color: theme.secondaryText, textAlign: "center", marginTop: 8 },
            ]}
          >
      ...
      >
        ...
        >
          ...
          <View style={{ marginTop: 20, marginBottom: 24 }}>
            ...
            >
              Enter a new password for your account. You'll use this to sign
              in next time instead of receiving a one-time code.
            </Text>
          </View>
